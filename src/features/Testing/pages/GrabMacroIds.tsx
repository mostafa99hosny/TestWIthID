...
const GrabMacroIds: React.FC = () => {
    ...
    const [macroIds, setMacroIds] = useState<[number, number][]>([]);

    // Step definitions for progress indicator
    const steps = [
        { step: 'report-id-input', label: 'Report ID', icon: List },
        { step: 'grabbing-in-progress', label: 'Grabbing Macros', icon: Download },
        { step: 'success', label: 'Success', icon: CheckCircle }
    ];

    // Handle macro IDs grabbing
    const handleGrabMacroIds = async () => {
        ...
        try {
            ...
            const result: any = await grabMacroIds(reportId, tabsNumValue);
            console.log("Macro IDs grab result:", result);

            setGrabResult(result);

            if (result.success) {
                const ids = result.data?.macro_ids_with_pages || [];
                ...
                <div className="bg-white rounded-2xl shadow-lg p-6">
                    ...
                    {currentStep === 'report-id-input' && (
                        <div className="space-y-6">
                            <StepHeader
                                ...
                                description="Provide the report ID and number of tabs to extract macro IDs from"
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex justify-center pt-2">
                                    <button
                                        ...
                                        disabled={!reportId.trim() || !tabsNum.trim() || isGrabbingMacros || !isLoggedIn}
                    ...
                    {currentStep === 'grabbing-in-progress' && (
                        <div className="space-y-6">
                            <StepHeader
                                icon={RefreshCw}
                                title="Grabbing Macro IDs"
                                description="Please wait while we extract macro IDs from the report..."
                                iconColor="text-blue-500"
                            />
                    ...
                    {currentStep === 'success' && (
                        <div className="space-y-6">
                            <StepHeader
                                icon={CheckCircle}
                                title="Success!"
                                description="Macro IDs have been extracted successfully"
                    ...
                    {currentStep === 'error' && (
                        <div className="space-y-6">
                            <StepHeader
                                icon={FileText}
                                title="Error"
                                description="There was an issue extracting macro IDs"
